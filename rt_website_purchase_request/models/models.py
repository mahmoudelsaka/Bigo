...
class PurchaseRequestUser(models.Model):
    _inherit = 'res.users'

    is_purchase = fields.Boolean('Show portal Menu')


class PurchaseRequestLine(models.Model):
    _inherit = 'purchase.order.line'

    gl_account_id = fields.Many2one(comodel_name='account.account', string="Gl Account")
...
class RtWebsitePurchaseRequest(models.Model):
    _inherit = 'purchase.request'

    def get_gl_account(self):
        print('========= TEST =============')
        gl_account_id = self.env['account.account'].sudo().search([('is_request', '=', True)])
        return gl_account_id


class UserPortalPO(models.Model):
    _inherit = "purchase.request.line"

    gl_account_id = fields.Many2one(comodel_name='account.account', string="Gl Account", required=True)

    def remove_record(self, rec_product):
        self.env['purchase.request.line'].sudo().search([('id', '=', int(rec_product))]).unlink()
        return True
