...
class Attachment(models.Model):
    ...
    attach_rel = fields.Many2many(comodel_name='purchase.request', relation='attachment', column1='attachment_id',
                                  column2='document_id', string="Attachment")
...
class PurchaseRequest(models.Model):
    ...
    type = fields.Many2one(comodel_name='purchase.request.type', string="Type", tracking=True)
    attachment_ids = fields.One2many(comodel_name='purchase.request.attachment', inverse_name="request_id",
                                     string="Attachments")

    attachment = fields.Many2many(comodel_name='ir.attachment', relation='attach_rel', column1='doc_id', column2='attach_id', string="Attachment",
                                  help='You can upload your document', copy=False)

    # @api.onchange('type')
    # def compute_line_product(self):
    #     for rec in self:
    #         if rec.type:
    #             product_type = rec.type.product_type
    #             if product_type:
    #                 if product_type == 'storable':
    #                     get_product = rec.env['product.product'].sudo().search([('detailed_type', '=', 'consu')])
    #                     if get_product:
    #                         rec.line_ids.product_id = get_product.ids
    #                 else:
    #                     get_product = rec.env['product.product'].sudo().search([('detailed_type', '!=', 'consu')])
    #                     if get_product:
    #                         rec.line_ids.product_id = get_product.ids
