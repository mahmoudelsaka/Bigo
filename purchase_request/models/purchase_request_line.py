...
class PurchaseRequestLine(models.Model):
    ...
    product_ids = fields.Many2many('product.product', string="Products", compute="_compute_product_type_ids")

    @api.depends('request_id.type')
    def _compute_product_type_ids(self):
        for rec in self:
            if rec.request_id.type:
                product_type = rec.request_id.type.product_type
                if product_type:
                    if product_type == 'storable':
                        products = rec.env['product.product'].sudo().search([('detailed_type', '=', 'product')])
                        rec.product_ids = products
                    else:
                        products = rec.env['product.product'].sudo().search([('detailed_type', '!=', 'product')])
                        rec.product_ids = products
                else:
                    products = rec.env['product.product'].sudo().search([])
                    rec.product_ids = products
            else:
                products = rec.env['product.product'].sudo().search([])
                rec.product_ids = products
