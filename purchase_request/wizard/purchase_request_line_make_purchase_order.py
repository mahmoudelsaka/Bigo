...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def make_purchase_order(self):
        ...
        for item in self.item_ids:
            line = item.line_id
            note = line.request_id.note
            type = line.request_id.type
            ...
            for d in domain:
                x = d[0]
                if x == 'analytic_distribution':
                    continue
                else:
                    domain2.append(d)
            available_po_lines = po_line_obj.search(domain2)
            new_pr_line = True
            ...
            if available_po_lines and not item.keep_description:
                ...
                if item.line_id.gl_account_id:
                    po_line_data['gl_account_id'] = item.line_id.gl_account_id.id
                if item.line_id.name:
                    po_line_data['name'] = item.line_id.name
