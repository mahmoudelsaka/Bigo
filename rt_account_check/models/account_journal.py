...
class AccountJournal(models.Model):
    ...
    def create(self, vals):
        ...
        issue_checks = self.env.ref(
            'rt_account_check.account_payment_method_issue_check')
        if rec.outbound_payment_method_line_ids:
            for line in rec.outbound_payment_method_line_ids:
                if (issue_checks in line.payment_method_id and not rec.checkbook_ids):
    ...
    @api.model
    def enable_issue_check_on_bank_journals(self):
    ...
    def open_action_checks(self):
        ...
        if check_type == 'third_check':
            action_name = 'rt_account_check.action_third_check'
        elif check_type == 'issue_check':
            action_name = 'rt_account_check.action_issue_check'
        else:
            return False
        ...
        return {
            ...
            'context': dict(
                ...
                default_payment_method_id=self.env.ref(
                    'rt_account_check.account_payment_method_issue_check').id,
            ),
        }
